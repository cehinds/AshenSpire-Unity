...
var progressed=(JObject)smithRun.DeepClone();progressed["progression"]=AttributeProgressionChecks.V1Rules();/* saved v1 runs keep the original five-point action gate over the fixture rules */playerProjection.Reconcile(progressed);Equal(progressed["energy"],new JValue(1+(int)attrs["dexterity"]/5),"Owner five-point action gate");

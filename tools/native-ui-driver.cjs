...
class NativeUiDriver {
 ...
 async choose(id,index){await this.click(id,false,.85);await this.key('Home');for(let n=0;n<index;n++)await this.key('ArrowDown');await this.key('Enter');await this.page.waitForTimeout(400);}

...
function buildPage(build,prefix){
 ...
 let shots=build.paths.filter(path=>/^Published\/Screenshots\/.*\.png$/.test(path)).slice(0,6);
 if(build.paths.includes('Published/presentation.json')){
  const presentation=JSON.parse(git(['show',`${build.commit}:Published/presentation.json`]));
 ...
 if(!manifest){body+='<p>No build has been selected for this channel yet.</p><p class="muted">Promotion is separate from publishing. The dev build is the current work in progress.</p>';summaries.push({channel,available:false});}
  ...
  if(plan.player.rewritten){
   ...
   for(const [name,url] of Object.entries(hosted.dataUrls)){
    ...
    let replaced=0;launch=launch.replace(pattern,(_,prefix,quote)=>{replaced++;return prefix+quote+url+quote;});
    if(replaced!==1)throw Error('Unsupported channel hosting URL: '+name);
   }
   mkdirSync(join(dir,'Web'),{recursive:true});writeFileSync(join(dir,'Web/index.html'),launch);
   writeFileSync(join(dir,'Web/channel-launch.json'),JSON.stringify({schemaVersion:1,channel,commit,archiveId:plan.archiveId,
    ...
    launchIndexSha256:createHash('sha256').update(launch).digest('hex'),runtime:hosted.receipt.runtime,reason:'Loader/framework/WebAssembly use the immutable Pages archive; data uses its exact Git commit. Document URL and save settings are unchanged.'},null,2));
...
const batchCards=[];
const sortedBatches=[...batches].sort((a,b)=>Date.parse(buildById.get(history.channels[a.id]?.at(-1))?.builtAt||0)-Date.parse(buildById.get(history.channels[b.id]?.at(-1))?.builtAt||0));
for(const batch of sortedBatches){
 const items=(history.channels[batch.id]??[]).map(id=>buildById.get(id));if(!items.length)continue;
 const dir=join(out,'batches',batch.id);mkdirSync(dir,{recursive:true});
 for(const build of items){const buildDir=join(dir,'builds',build.id);mkdirSync(buildDir,{recursive:true});writeFileSync(join(buildDir,'index.html'),page(`${batch.branch} build ${build.buildNumber}`,buildPage(build,'../../../../')));}
 const latest=items.at(-1);
 writeFileSync(join(dir,'index.html'),page(batch.branch,`<nav><a href="../../">Build library</a><a href="../">All batches</a></nav><p class="kicker">Development branch</p><h1>${escape(batch.branch)}</h1><p>These committed previews have not necessarily been promoted to dev or test. Follow the CI run and PR for validation status.</p><a class="button secondary" href="${repo}/actions?query=${encodeURIComponent('branch:'+batch.branch)}">CI pipeline and results</a><section class="grid">${items.map((build,i)=>buildCard(build,'',items[i-1])).reverse().join('')}</section>`));
 batchCards.push(`<article class="card"><p class="kicker">Build ${escape(latest.buildNumber)}</p><h2>${escape(batch.branch)}</h2><p>${escape(latest.manifest.version)} · ${escape(dateLabel(latest.builtAt))}</p><a class="button" href="${batch.id}/">Play and browse this batch</a>${prLinks(latest)}</article>`);
}
mkdirSync(join(out,'batches'),{recursive:true});writeFileSync(join(out,'batches/index.html'),page('Development batches',`<nav><a href="../">Build library</a></nav><h1>Development batches</h1><p>Every committed browser build, grouped by its development branch. A playable preview is distinct from a passed CI run or channel promotion.</p><section class="grid">${batchCards.reverse().join('')}</section>`));
writeFileSync(join(out,'batches.json'),JSON.stringify(batches.filter(batch=>history.channels[batch.id]?.length),null,2));

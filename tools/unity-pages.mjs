...
for(const channel of channels){
 ...
 if(!manifest){body+='<p>No build has been selected for this channel yet.</p><p class="muted">Promotion is separate from publishing. The dev build is the current work in progress.</p>';summaries.push({channel,available:false});}
  ...
  const screenshots=paths.filter(p=>/^Published\/Screenshots\/.*\.png$/.test(p)||/^Published\/ClassEvidence\/[^/]+\/(04-phone-combat|10-reward-0|11-next-map|15-campaign-defeat|26-action-impact)\.png$/.test(p)||/^Published\/FeedbackEvidence\/(26-normal-impact|26-normal-impact-settled|28-reduced-impact|31-interrupted-menu)\.png$/.test(p));

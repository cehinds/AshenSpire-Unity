...
(async () => {
  ...
  check(creations.at(-1).classId === 'reaver' && creations.at(-1).attributes.strength === 13, 'original tuned Reaver preset');
  check(creations.at(-1).resources.hp === 52 && creations.at(-1).resources.energy === 3, 'native original base resources');

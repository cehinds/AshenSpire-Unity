...
(async()=>{
 ...
 await ui.command(ui.controls.Controls.find(c=>c.Id.startsWith('native-draft-')&&c.Enabled).Id);
 ...
 while(ui.state.phase==='Draft')await ui.command(ui.controls.Controls.find(c=>c.Id.startsWith('native-draft-')&&c.Enabled).Id);

...
namespace AshenSpire.Presentation
{
    public sealed class OriginalFoundationPanel
    {
        ...
        private void Header(string title)
        {
            ...
            Button("foundation-creation", "Wanderer & attributes", Creation);
            Button("foundation-map", "Explore the original routes", Map);
            Button("foundation-catalog", "Browse the original content", Catalog);
            Button("foundation-back", "Back to title", _back);
        }
        private void Creation()
        {
            Header("THE ORIGINAL WANDERERS");
            Select("foundation-class", "Class", _catalog.Table("classes").Select(x => (string)x["id"]).ToList(), _creation.ClassId, value => { _creation.Select(value, _creation.ModeId); Creation(); });
            Select("foundation-mode", "Allocation", _catalog.Table("creationModes").Select(x => (string)x["id"]).ToList(), _creation.ModeId, value => { _creation.Select(_creation.ClassId, value); Creation(); });
        ...
        private void Map()
        {
            ...
            var seed = new TextField("Seed") { value = _seed, name = "foundation-seed" }; seed.RegisterValueChangedCallback(e => _seed = e.newValue); _root.Add(seed);
        ...
        private void Catalog()
        {
            ...
            var query = new TextField("Find name or ID") { value = _search, name = "foundation-search" }; query.RegisterValueChangedCallback(e => _search = e.newValue); _root.Add(query);
        ...
        private void Detail(JObject row)
        {
            ...
            var json = new TextField("Original record") { value = row.ToString(), multiline = true, isReadOnly = true, name = "foundation-record" }; json.AddToClassList("report-field"); _root.Add(json);
        ...
        private void Label(string text, string style) { var label = new Label(text); label.AddToClassList(style); _root.Add(label); }
